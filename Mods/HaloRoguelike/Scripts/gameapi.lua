...
-- Read a tag container. BreakGameplayTagContainer returned nothing usable
-- on-device (its out-param array came back nil under every convention tried),
-- so the container is read as a STRING instead: GetDebugStringFromGameplayTag-
-- Container returns an FString, and GetNumGameplayTagsInContainer an int. Both
-- are plain scalar returns with nothing to marshal, which is exactly why they
-- work where the array API did not.
local function container_debug(container)
    if container == nil then return nil, 0 end
    local lib = tag_library()
    if not (lib and lib:IsValid()) then return nil, 0 end
    local text = try("GetDebugStringFromGameplayTagContainer", function()
        return fstr(lib:GetDebugStringFromGameplayTagContainer(container))
    end)
    local count = tonumber(try("GetNumGameplayTagsInContainer", function()
        return lib:GetNumGameplayTagsInContainer(container)
    end)) or 0
    return text, count
end

-- Tag names out of a container's debug string. The string is a comma-separated
-- list, so this is parsing rather than reflection — acceptable because it is
-- only used to report and to verify, never to build.
local function read_tag_container(container)
    local text = container_debug(container)
    if not text or text == "" then return nil end
    local out = {}
    for word in text:gmatch("[%w_%.]+") do out[#out + 1] = word end
    if #out == 0 then return nil end
    return out
end
...
function Gameapi.dump_skull_tags()
    ...
    local text, count = container_debug(container)
    Log.discover("tags: ACTIVE SKULL TAGS (%d): %s", count, tostring(text))
    local names = read_tag_container(container)
    if not names then
        Log.discover("tags: the container reported no tag names — "
            .. "GetDebugStringFromGameplayTagContainer returned %s",
            tostring(text))
    end
